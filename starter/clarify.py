...
-------------------------------------------------------------------------
ASSUMED CONTRACTS (confirm these against the real state.py / retrieval.py
before wiring in — these are inferred from the PRD, not yet locked code):

state (dict-like, owned by Person B / state.py) must expose:
    state["filled_slots"]      : dict[str, str|None]   # 9 categories
    state["filled_null"]       : set[str]               # explicit no-pref
    state["asked_categories"]  : set[str]                # already asked
    state["turn"]               : int

candidate (owned by Person A / retrieval.py) must expose, per item in the
candidate_pool list:
    candidate["parent_asin"]   : str
    candidate["attrs"]         : dict[str, str|None]     # parsed per-category
                                   values for this product, same 9 keys
    candidate["score"]          : float                   # retrieval score

If A's candidates come back as objects instead of dicts, swap the
`_get` helper below for attribute access — nothing else needs to change.
-------------------------------------------------------------------------
"""
...
def pick_attribute_to_ask(
    ...
    top_k: int,
    boundary_detected_this_turn: bool = False,
) -> Optional[str]:
    """
    ...
    """
    filled_slots: dict = state.get("filled_slots", {})
    filled_null: set = state.get("filled_null", set())
    asked_categories: set = state.get("asked_categories", set())
...
if __name__ == "__main__":
    ...
    fake_state = {
        "filled_slots": {cat: None for cat in ALL_NINE_CATEGORIES},
        "filled_null": set(),
